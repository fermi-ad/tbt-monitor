...
- Notes: plane-specific N is allowed because H and V are independent diagnostics. The mixed four-panel image still uses one shared P98-clipped color scale, but visual thickness is compared only within each plane because the tune-band widths differ. Per-turn contrast CSVs are unsmoothed; five-window plot smoothing is visual only, and no contrast metric is labeled as physical noise or extraction timing.
