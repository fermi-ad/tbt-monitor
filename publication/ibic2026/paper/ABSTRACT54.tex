...
Within those limits, adaptive ensembles are preferable to a single preferred BPM or static small list when channel identity and quality diagnostics are required. All-training aggregation should remain an explicit same-protocol control. A practical monitor should expose the selected exact channels, ensemble size, tune candidate, prominence and ambiguity measures, later-window support, and a no-reliable-tune state. Horizontal and vertical quality decisions should remain independent.

\section{Conclusion}

Adaptive BPM ensembles recover repeatable tune-like spectral structure. Leakage-controlled, digitizer-disjoint validation selects a useful size knee relative to Best-1; vertical evidence is strongest and horizontal tracking remains a limitation. Frozen sets and intensity weighting do not replace adaptive selection, while same-protocol all-training outcomes are explicit. This BPM-only internal diagnostic still requires external tune calibration.
