...
Dynamic Best-5 exceeds frozen Best-5 vertically under this descriptive score, but the all-BPM median control scores above both small-set methods in each plane; vertically, all-BPM mean does as well. The leakage-controlled result therefore supports improvement over adaptive Best-1 and reproducibility of the selected knee; it does not establish superiority to all-BPM aggregation.
...
Within those limits, adaptive ensembles are preferable to a single preferred BPM or static small list when channel identity and quality diagnostics are required. All-BPM aggregation should remain an explicit baseline. A practical monitor should expose the selected exact channels, ensemble size, tune candidate, prominence and ambiguity measures, later-window support, and a no-reliable-tune state. Horizontal and vertical quality decisions should remain independent.

\section{Conclusion}

Synchronized distributed BPM streams can recover repeatable tune-like spectral structure when small ensembles are selected spill by spill. Leakage-controlled, digitizer-disjoint validation determines a useful ensemble-size knee relative to Best-1 rather than assuming that more BPMs are better. The strongest result is vertical; the horizontal plane remains a diagnostic limitation. Frozen small sets and intensity weighting do not replace adaptive selection, while all-BPM aggregation remains a strong unvalidated baseline. The method is therefore a defensible BPM-only tune-candidate diagnostic and a foundation for later externally calibrated tune monitoring.
