...
The primary data set contains \PrimarySpillCount{} captured spills in two acquisition collections. Its nominal topology supplies \PrimaryNominalHChannels{} horizontal and \PrimaryNominalVChannels{} vertical position channels, with two same-plane sources per digitizer; \PrimaryPartialCaptures{} flagged partial captures contain \PrimarySourceAbsences{} recorded source absences. Absent channels remain missing rather than being fabricated or zero-filled. Each recorded waveform contains approximately 50,000 turns. The analysis consumes raw host-published position arrays: ``raw'' means DSP-derived position floats before host-side threshold substitution and scaling, not electrode ADC samples. Stream timestamps are used to assemble a coherent spill snapshot. Adjacent target buckets are merged within \qty{1}{ms}, while captured streams are checked against the selected target using the separately configured \qty{25}{ms} same-spill tolerance. Incomplete or inconsistent states carry quality flags rather than being silently accepted.
...
To visualize persistence through the approximately 50,000-turn record, the selected early-window membership is applied to the raw captured spill. Each selected trace is RMS-normalized, 4,096-turn Hann spectra are computed every 256 turns, and selected-BPM powers are averaged. One locally tracked ridge pick per spill and window is binned across spills. This plot is a density of selected ridge locations, not a spectral-power heat map. Of \RidgeHStructuralRows{} structural rows per plane, H retains \RidgeHFinitePicks{} finite in-band picks (\RidgeHBlankPicks{} blank and \RidgeHEdgeExcludedPicks{} bounded edge exclusions), while V retains \RidgeVFinitePicks{} (\RidgeVBlankPicks{} blank and \RidgeVEdgeExcludedPicks{} edge exclusions); none is imputed.
...
\begin{figure*}[!htb]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/ridge_width_contrast_hv.png}
  \caption{Exact-paired selected-Best-$N$ minus corrected-Best-1 P10--P90 ridge-pick width; negative values indicate a narrower selected-ensemble distribution. Panels share one y scale, and curves are five-window visual smooths of unsmoothed exports. This isolates ensemble size, not physical noise removal, absolute tune accuracy, or extraction timing.}
...
Dynamic Best-5 exceeds frozen Best-5 vertically under this descriptive score, but all-BPM aggregation remains strong. A second, leakage-controlled comparison therefore aggregates every training-side channel by mean and median under the same purged later windows and held-out-digitizer folds as Best-$N$. Of eight method--metric comparisons per plane, selected Best-$N$ is favored in \AllTrainingHSelectedFavored{} H and \AllTrainingVSelectedFavored{} V comparisons, all-training aggregation is favored in \AllTrainingHBaselineFavored{} H and \AllTrainingVBaselineFavored{} V comparisons, and \AllTrainingHUnresolved{} H plus \AllTrainingVUnresolved{} V comparisons remain unresolved. This is an internal reproducibility control, not an external tune calibration.
...
The study has four principal limits. First, internal BPM agreement is not an independent tune calibration. Second, unlabeled machine states limit block-aware uncertainty; controlled scans remain necessary. Third, fixed early-window membership makes the 50,000-turn visualization a persistence test, not full-spill reselection. Fourth, ridge concentration can improve if all methods follow the same biased candidate. A matched Schottky or tune-meter reference and controlled tune-knob study are therefore required.

Within those limits, adaptive ensembles are preferable to a single BPM or static list when channel identity and quality diagnostics matter. All-training aggregation remains an explicit same-protocol control. An operational monitor should expose exact members, ensemble size, tune candidate, prominence and ambiguity, later-window support, and a no-reliable-tune state, with plane-independent quality decisions.

\section{Conclusion}

Adaptive BPM ensembles recover repeatable tune-like structure. Leakage-controlled, digitizer-disjoint validation selects a useful size knee relative to Best-1; vertical evidence is strongest and horizontal tracking remains limited. Frozen sets and intensity weighting do not replace adaptation, while same-protocol all-training outcomes remain explicit. This BPM-only diagnostic still requires external tune calibration.
